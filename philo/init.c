...
/*   Updated: 2025/05/12 18:10:27 by zbouchra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_data2(t_philo **philos, t_pdata **pdata)
{
	...
	*philos = malloc(sizeof(t_philo) * (*pdata)->noph);
	...
	(*pdata)->forks = malloc(sizeof(pthread_mutex_t) * (*pdata)->noph);
	if (!(*pdata)->forks)
		return (ft_destroy(*philos, *pdata, "Error: Memory allocation failed\n"));
	i = 0;
	while (i < (*pdata)->noph)
	{
		if (pthread_mutex_init((&(*pdata)->forks[i]), NULL) != 0)
			return (ft_destroy(*philos, *pdata, "Error: Mutex initialization failed\n"));
		i++;
	}
...
int	handle_args(t_philo **philos,t_pdata **pdata, int c, char **v)
{
	int	err;
	...
	if ((*pdata)->noph <= 0 || err)
		return (ft_destroy(*philos, *pdata, "Error: Invalid number of philos\n"));
	(*pdata)->time_to_die = ft_atol(v[2], &err);
	if ((*pdata)->time_to_die <= 0 || err)
		return (ft_destroy(*philos, *pdata, "Error: Invalid time to die\n"));
	(*pdata)->time_to_eat = ft_atol(v[3], &err);
	if ((*pdata)->time_to_eat <= 0 || err)
		return (ft_destroy(*philos, *pdata, "Error: Invalid time to eat\n"));
	(*pdata)->time_to_sleep = ft_atol(v[4], &err);
	if ((*pdata)->time_to_sleep <= 0 || err)
		return (ft_destroy(*philos, *pdata, "Error: Invalid time to sleep\n"));
	if (c == 6)
	{
		(*pdata)->number_of_times_to_eat = ft_atol(v[5], &err);
		if ((*pdata)->number_of_times_to_eat <= 0 || err)
			return (ft_destroy(*philos, *pdata, "Err: Invalid num of times to eat\n"));
	}
	else
...
int	init_data(t_philo **philos, t_pdata **pdata, int c, char **v)
{
	...
	*pdata = malloc(sizeof(t_pdata));
	...
	if (handle_args(philos ,pdata, c, v))
		return (1);
